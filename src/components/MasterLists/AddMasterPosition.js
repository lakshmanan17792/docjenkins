...
import { PositionValidator } from './MasterListsValidation.js';
...
@reduxForm({
  form: 'AddMasterPosition',
  validate: PositionValidator,
  touchOnChange: true
})
export default class AddMasterPosition extends Component {
  static propTypes = {
    ...
    return (
      ...
      >
        <Modal.Header className={`${styles.modal_header_color}`}>
          <Modal.Title>
            {isEdit ? i18n.t('UPDATE_POSITION') : i18n.t('ADD_POSITION')}
            <span
              role="button"
              tabIndex="-1"
        ...
        <form name="AddPositionForm" onSubmit={handleSubmit}>
          <Modal.Body>
            <Field
              name="name"
              type="text"
          ...
          <Modal.Footer>
            ...
            >{isEdit ? i18n.t('UPDATE_POSITION') : i18n.t('ADD_POSITION')}</button>
            <button
              type="button"
